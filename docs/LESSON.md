...
- **CSV**: formula-neutralize fields starting with `= + - @ \t \r` (prefix `'`); RFC-4180 quote/escape `" , \n`. Prepend a UTF-8 BOM in `downloadCsv` (not in the pure `toCsv`) so Excel renders €/accents while unit tests keep a clean string. `exportReview.ts` and the grid mirror share one pure cell-text function, `cellDisplayText` in `grid/format.ts` (rating `n/5`, `—` for null, empty for pending) — single source of truth, unit-tested, no duplication/drift.
